...
export class GoogleMapsService {
  ...
  async getETA(origin: { lat: number; lng: number }, destination: { lat: number; lng: number }): Promise<number> {
    ...
    this.logger.debug(`Calculated fallback ETA: ${eta} mins for ${distance.toFixed(0)}m`);
    return eta;
  }
